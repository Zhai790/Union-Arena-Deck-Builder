...
export function validateDeck(
  ...
  const copyValidations = validateCopyLimit(deck, cardIndex);
  for (const validation of copyValidations) {
...
export function getValidationSummary(result: ValidationResult): string {
  ...
  if (result.valid && warningCount > 0) {
    return `✓ Deck is legal (${warningCount} warning${warningCount === 1 ? '' : 's'})`;
  }
